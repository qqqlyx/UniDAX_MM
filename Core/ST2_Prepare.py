...
读取参数文件,遍历对应文件夹
'''
_path = Tokens._path + '\\ST_setting\\UniDAX_MarketMaker.xlsx'
datas = pd.read_excel(_path)

# 函数
def prepare_NextDayPlan():
    ...
    # 控制交易量
    # 整体交易量在2个月内下降60%
    control_begin_date = datetime.datetime.strptime('2020-02-20', "%Y-%m-%d")
    ...
    control_speed = [62.3, 64.1,65.1,67.3,69.2,72.4,74.7,78.1,80.2,81.3,83.2,84.1]
    ...
    # 写入文件
    writePath = Tokens._path + '\\ST_setting\\' + str(nextDAY.year) + str(nextDAY.month) + str(nextDAY.day)
    ...
    # 写入其他参数
    f = open(writePath + '\\Param.txt', 'w')
    f.write('HuobiRatio=' + str(_totalTradeAmount_Huobi))

    trade_Time = int((p_trade_time[0] + p_trade_time[1]) / 2)
    f.write('\np_trade_time=' + str(trade_Time))
    f.write('\np_total_amount=' + str(_totalTradeAmount))
